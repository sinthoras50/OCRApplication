...
//            Core.rotate(modified, modified, Core.ROTATE_90_CLOCKWISE);
        ...
        Log.d("MAT_SIZE", "Bitmap - width: " + mCacheBitmap.getWidth() + " height: " + mCacheBitmap.getHeight());
        Log.d("MAT_SIZE", "Mat - width: " + modified.width() + " height: " + modified.height());
        ...
        if (bmpValid && mCacheBitmap != null) {
            ...
            if (canvas != null) {
                ...
                // make the frame fit the entire screen, commented out lines can rotate/scale the bitmap if mat wasnt rotated to fit the portrait mode
                Matrix matrix = new Matrix();
                // translate to middle allowing for correct scaling/rotation
...
//                float scale = (float) canvas.getWidth()/(float) mCacheBitmap.getHeight();
                ...
                // calculate empty space on top so that we can push the image to the top leaving space on the bottom for controls
                float top = (0.5f * (canvas.getHeight() - (mCacheBitmap.getHeight()*scale))) / scale;
//                float top = (0.5f * (canvas.getHeight() - (mCacheBitmap.getWidth()*scale))) / scale;

                matrix.preTranslate(0, -top);
